...
public interface PrestamoRespository extends JpaRepository<Prestamo, Long> {
    ...
    // ADMIN
    @Query("select p from Prestamo p join fetch p.id_usuario pe join fetch p.id_libro li join fetch p.id_empleado em where em.local.id=?1")
    ...
    @Query("select p from Prestamo p join fetch p.id_usuario pe join fetch p.id_libro li join fetch p.id_empleado em where em.id=?1 or em.id=1")
    ...
    @Query("select p from Prestamo p join fetch p.id_usuario pe join fetch p.id_libro li join fetch p.id_empleado em where li.id=?1")
    ...
    @Query("select p from Prestamo p join fetch p.id_usuario pe join fetch p.id_libro li join fetch p.id_empleado em where li.id=?1 and li.local.id=?2")
    ...
    @Query("select p from Prestamo p join fetch p.id_usuario pu join fetch p.id_libro pli join fetch p.id_empleado pem where pu.id=?1 and p.isActivo=1")
    ...
    @Query("select p from Prestamo p join fetch p.id_usuario pu join fetch p.id_libro pli join fetch p.id_empleado pem where pu.id=?1 and pem.usuario like '%Prueba%' and p.isActivo=0")
